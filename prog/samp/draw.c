/* Visualize clusters generated by `mds.c'
 *
 * This program uses OpenGL to render clusters on screen.
 * Then, when the user presses `p', it saves the screen
 * into a PNG file using Imlib2.
 * By default, the output file share the same name with
 * the input file, except that the extension is changed to .png
 *
...
static double getscale(real *x, int n, int dim)
{
  ...
  for ( i = 0; i < n * dim; i++ )
    if ((xm = fabs(x[i])) > xmax)
      xmax = xm;
  return xmax + .5; /* add a radius as the margin */
}
