    ...
    int forward, FFTWPFX(plan) p)
{
  ...
  if ( p == NULL ) { /* create a default plan if necessary */
    p = FFTWPFX(plan_r2r_1d)(npt, out, out, FFTW_RODFT11, FFTW_ESTIMATE);
    noplan = 1;
  }
  ...
  for ( i = 0; i < npt; i++ ) /* form out(k) / k */
    out[i] *= fac * 2 / (2*i + 1);
  if ( noplan ) FFTWPFX(destroy_plan)(p);
}
...
static int intgeq(int nmax, int npt, mydouble dr, int doHNC)
{
  ...
  mydouble **ck, **tk, *fr, *fk, *dfr, *tl, *cc, *Bc, *Bv;
  int i, l, u, j, jl, k;
  ...
  sphr(npt, fr, fk, dr, 1, NULL); /* f(r) --> f(k) */
  ...
  for ( l = 1; l < nmax - 1; l++ ) {
    ...
    if ( doHNC ) {
      ...
      for ( i = 0; i < npt; i++ ) cc[i] = (1 + fr[i]) * yr[l][i] - tl[i];
       ...
       * c(r) = f(r) (1 + t(r)) */
      for ( i = 0; i < npt; i++ ) cc[i] = fr[i] * tl[i];
      for ( Bv[l+2] = 0, i = 0; i < npt; i++ )
