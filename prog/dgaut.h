...
INLINE void dg_permutate(dg_t * RESTRICT gout, const dg_t * RESTRICT gin, const int *perm)
{
  DG_DEFN_(gin);
  int i, j;
  code_t ci, ci_out;
  ...
  for (i = 0; i < DG_N_; i++) {
    for (ci = gin->c[i], ci_out = 0, j = 0; j < DG_N_; j++, ci >>= 1)
      if (ci & 0x1)
        ci_out |= MKBIT(perm[j]);
    //die_if (perm[i] < 0 || perm[i] >= DG_N_, "perm(%d) = %d\n", i, perm[i]);
...
/* get a representative graph `gout' of the input graph `gin'
 ...
 * */
INLINE dg_t *dg_getrep(dg_t *gout, const dg_t *gin, int level)
{
  int perm[DG_NMAX + 1];
  ...
  //{ int i; for (i = 0; i < gin->n; i++) die_if (perm[i] < 0 || perm[i] >= gin->n, "perm(%d) = %d\n", i, perm[i]); }
