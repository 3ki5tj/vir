...
/* convert the graph generated by dg_repiso() to code */
INLINE void dg_repisocode(dgword_t *code, const dg_t *g, int level)
{
  static dg_t *ng;
#pragma omp threadprivate(ng)

  if (ng == NULL) ng = dg_open(DG_NMAX);
  dg_repiso(ng, g, level);
  dg_encode(ng, code);
}
...
INLINE int dg_repisocodels(dgword_t *codes, int cwords, int nmax,
    const dg_t *g, int level)
{
  dgpart_t part;
  int ncnt, i, ic, j, n = g->n;
  static int *perms, permcap; /* permutation */
  static dg_t *gout;
#pragma omp threadprivate(perms, permcap, gout)
  ...
  if (gout == NULL) gout = dg_open(DG_NMAX);
  /* obtain a list of possible permutations */
  ncnt = dg_part2permls(perms, nmax, &part);
  /* convert each permutation to a code */
  for (ic = 0, i = 0; i < ncnt; i++) {
    //printf("i %d, ", i); dg_printperm(perms + i * n, n);
    dg_permutate(gout, g, perms + i * n);
    //dg_print(gout);
    dg_encode(gout, codes + ic * cwords);
