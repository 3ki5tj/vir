...
%  {
%    \begin{tikzpicture}
%      \node (r1) at (180 : \sz) [whitedot]{};
%      \node (r2) at (  0 : \sz) [whitedot]{};
%      \draw (r1) -- (r2);
%    \end{tikzpicture}
%  }
%  +
...
%    \cdots \\
%  \hphantom{wwwwww}
%    d\vr^{(l-1)} \, a_{l-1}(\vr^{(l-1)}) \,
%    a_l(\vr - \vr^{(1)} - \vr^{(2)} - \cdots - \vr^{(l-1)}).
%\end{multline*}
...
%  rooted at $v$ and $n$,
%%
%  and recursively repeating the above step
%  shows that
%  $G^{(2)}$ is a chain of polygon graphs.
%%
%In either case, $G = G^{(1)} * G^{(2)}$
%  is a chain of polygon graph.
...
\begin{equation*}
  ...
  %
    ...
    %  coordinates{(-0.75*\sz, 0) (-0.4*\sz, -0.67*\sz) (0.75*\sz, 0) (0.4*\sz, 0.67*\sz)};
...
%
%Multiplying \eqref{eq:ccyclintg} by
%  $2$, $l$, and $l(l-3)$
...
%The $c$-graph characterizes $c(\vr)$ in the HNC case,
%  just like the polygon graph in the PY case.
%%
%By definition, a $c$-pack can be written as
%  as a cycle of $c$-graphs,
%  and the representation is not unique.
%%
%In the PY case,
%  each cycle corresponds to a face of the polygon.
%%
%Below we develop an analogy for the HNC case,
%  which helps generalizing the counting formulas
...
%  that is not an $f$-graph, into a cycle $J''$.
%%
%To avoid decomposing the link
