...
\subsection{Graphic interpolation of the closure}
...
(1) cycling and reflecting the cycle elements, and
(2) exchanging the vertices in each $G_i$.
...
\[
  y(\vr) = \exp t(\vr) = 1 + t(\vr) + t^2(\vr)/2 + \cdots,
\]
every graph $M$ in the graphic expansion of $y(\vr)$,
  specifically, in the expansion of $t^m(\vr)/m!$ ($m \ge 0$),
...
\begin{equation}
  ...
  \label{eq:Adef}
\end{equation}
The sums obtained from whitening all possible pivots,
adjacent pivots, and nonadjacent pivots
...
By using Lemma \ref{thm:crsplit},
  we can separate $c$-graphs with adjacent roots from nonadjacent ones,
  and this yields the other two sums.
\end{proof}



Lemma \ref{thm:whitenA} can be
...
%
Thus, Lemma \ref{thm:whitenA} can now be stated as
\begin{align*}
  \frac{\rho^2}{2} c(\vr)
&=
  \left.
...
in the HNC closure uses the same weight as
...
produce the same virial coefficients as the virial-route pressure formula:
\begin{equation}
  \beta \, \Pexv
=
  - \frac{\rho^2}{2D}
...
The integral can be written in reduced coordinates
$\vR_i = V^{-1/D} \vr_i$
as
\begin{align*}
  I^*(G; \rho, \zeta)
&= N^{n-1} \int \prod f(\zeta \, V^{1/D} \vR_{ij}) \, d\vR_2 \cdots d\vR_n
  \notag \\
&= I^*(G; N, \zeta \, V^{1/D}) \\
&= I^*(G; N, N^{1/D} \, \zeta \, \rho^{-1/D}),
\end{align*}
where
$N = V \rho$ is the number of particles
(not to be confused to with the order $n$).
%
It follows that
\begin{align}
...
\end{align*}
%or in terms of the graphic sums,
%\begin{align*}
...
\subsection{\label{sec:vtpy}Simplified Baxter's pressure formula (PY closure)}
...
in which \emph{every} $f$-bond in $c(\vr; \zeta)$
is affected by $\zeta$.
...
\subsection{\label{sec:vthnc}Virial-route nature of \eqref{eq:hncF} and \eqref{eq:hncmu} (HNC closure)}
...
\end{align}
%
where we have used \eqref{eq:dTdzeta}, \eqref{eq:dfdzeta}, %, \eqref{eq:Pvirial},
and
\[
  \partial_\zeta c
