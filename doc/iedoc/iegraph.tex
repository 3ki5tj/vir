...
\begin{tabular}{p{1.3cm} p{1.2cm} c l r p{1.6cm} c l}
  ...
  \multicolumn{8}{p{\textwidth}}{
    $^\ast$
    The sum is over the respective rootless graphs $G$,
...
all 2-root unlabeled biconnected graphs $G$
weighted by $1/s(G)$.
...
Further,
  ...
  by permutations in the dihedral group,
...
Note that the $n = 2$ case is special,
  for the rootless $f$-graph,
  \begin{tikzpicture}
    \node (r1) at (-3mm, 0) [blackdot]{};
    \node (r2) at ( 3mm, 0) [blackdot]{};
    \draw (r1) -- (r2);
  \end{tikzpicture},
  does not form a polygon.
Thus, $s(G)$ in Eq. \eqref{eq:pykappasum}
takes the special value of $4$ instead of $2$
in this case.
...
\subsection{Baxter's pressure formula for the PY equation}


%We now derive Baxter's pressure formula
%\cite{baxterpressure}.
%
To obtain an expression of pressure for the PY equation,
...
%
integrating over $\rho$ yields\cite{baxterpressure}
%
\begin{align}
      -\beta \Pex
...
\[
  \newcommand{\sz}{3mm}
  \begin{tikzpicture}
    \node (v1)    at ( 0, 0) [whitedot,label=left:$v_1$]{};
    ...
    \node (vl)    at (10.0*\sz, 0) [whitedot,label=right:$v_l$]{}
...
\end{align*}
where we have used Eq. \eqref{eq:yrfunctional} on the second line.
%
...
To proceed, we need to generalize the face decomposition in the PY case
  ...
  which helps adapt the counting formulae,
...
We will show below that
  ...
  or
  (b) $L^{(s)}$ and $J$ share the same set of joints.
%
...
If (a) is not true,
  it means that the vertices in $V_L$
...
\[
  \newcommand{\sz}{4mm}
  \begin{tikzpicture}
    \node (v1)    at ( 1.5*\sz, 0) [blackdot,label=left:$v_1$]{};
    \node (v2)    at ( 2.1*\sz, 2.0*\sz) [blackdot,label=left:$v_2$]{}
      edge[thick, bend right=10] (v1);
    \node (ui)  at ( 2.0*\sz, 3.7*\sz) [blackdot,blue,label=left:$u_{i}$]{};
    \node (uip1)  at ( 8.0*\sz, 3.7*\sz) [blackdot,blue,label=right:$u_{i+1}$]{}
      edge[blue, thick, bend right=15] (ui);
    \node (vlm1)  at ( 7.9*\sz, 2.0*\sz) [blackdot,label=right:$v_{l-1}$]{}
      edge[dotted, bend right=70] (v2);
    \node (vl)    at ( 8.5*\sz, 0) [blackdot,label=right:$v_l$]{}
    ...
    \node (u2)    at ( 2.0*\sz, 8.0*\sz) [blue, blackdot,blue,label=135:$u_2$]{}
      edge[blue,dotted,thick, bend right=60] (ui)
      edge[blue,thick, bend left=8] (u1);

    % shading of the L circle
    \draw[fill=white!20!gray, opacity=0.2, draw=blue]
      plot [smooth cycle]
      coordinates{
                   ( 2.0*\sz,  3.7*\sz)
                   ( 5.0*\sz,  4.5*\sz)
    ...
    \node (casea) at (5.0*\sz, 11*\sz) {Case (a).};
    % caption
  \end{tikzpicture}
%
\quad
%
  \begin{tikzpicture}
    ...
    \node (caseb) at (5.0*\sz, 11*\sz) {Case (b).};
    % caption
  \end{tikzpicture}
\]

By repeating the above argument on $u_{i-1}$ and/or $u_{i+1}$,
whichever belong(s) to $V_L$,
we can show that
  all the vertices in $V_L$,
  $v_2, \dots, v_{l-1}$,
  hence all the joints $v_1$, \dots $v_l$ of $L^{(s)}$,
  are also consecutive joints of $J$.
%
This means that the two cycles share the same set of joints
  [case (b)].
\end{proof}
...
Given a $c$-pack $G$,
  ...
  discovered in Step 2 of Algorithm D.
%
Counting vertices and vertex pairs
  are readily established on the simple cycles.
%
First, Step 3 of Algorithm D removes $(l - 2)$ vertices,
...
\begin{align}
  ...
  &=
      \sum_G \frac{ p(G) \, I(G) } { s(G) }
      -
      \sum_{l = 3}^\infty \left[ l \, (l - 1) - 2 \right] \, \Chn_1
      \notag \\
  &=
      \rho^2 \int c(\vr) \, d\vr
    - \sum_{l = 3}^\infty \left[ l \, (l-3) + 2 \, l - 2 \right] \, \Chn_l
      \notag \\
  &=
...
where,
  on the second line,
  we have used
  Eq. \eqref{eq:countpairs} on the second line,
  Eqs. \eqref{eq:hnccrsum} on the third,
  and Eqs. \eqref{eq:Clsum3}-\eqref{eq:Clsumll} on the last line.
  %with $l\, (l-1)-2 = l \, (l-3) + 2 \, l - 2$.
%
...
%
we have used
Eq. \eqref{eq:countvertices} on the second line,
Eq. \eqref{eq:countpairs} on the third,
Eq. \eqref{eq:hnccrsum} on the fourth,
and Eq. \eqref{eq:Clsum_th} on the last.
...
Here we derive a general virial theorem,
and use it
...
whereas the rest two equations result from the PY closure,
Eq. \eqref{eq:py}.
%
Thus,
\begin{align}
...
We should caution the reader about a possible confusion of notations
...
If the pair is on a side of a polygon,
  ...
  i.e., the symmetry number $s$ is doubled
  (cf. the comment after Lemma \ref{thm:pykappasum}).
%
Otherwise,
...
\end{align*}
%
where the leading factor, $\frac 1 2$,
is due to the two orderings of the two vertices in a pair.
%
